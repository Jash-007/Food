...
router.get('/searchfoodData/:search', async (req, res) => {
    try {
        ...
        if (query.trim() === 'all') {
            ...
            items = await mongoose.connection.db.collection("food_items").find({ 'name': { '$regex': query } }).toArray();
        }
...
router.get('/adminview', async (req, res) => {
    try {
        ...
        //res.send({foodData:global.foodData, foodCategory:global.foodCategory,user: userdata})
